...
namespace EDDiscovery
{
    public partial class EDDiscoveryForm : Form
    {
        ...
        private static void GetNewRedWizzardFile(string filename, string url)
        {
            ...
            {
                string etag = File.ReadAllText(etagFilename);
                if (etag != "")
                {
            ...
            try {
                ...
                TravelHistoryControl.LogText("Downloading " + filename + "..." + Environment.NewLine);

                File.WriteAllText(filename + ".etag.tmp", response.Headers[HttpResponseHeader.ETag]);
                FileStream destFileStream = File.Open(filename + ".tmp", FileMode.Create, FileAccess.Write);
            ...
            } catch (System.Net.WebException e)
            {
                ...
                {
                    TravelHistoryControl.LogText(filename + " is up to date." + Environment.NewLine);
                } else
                {
                    throw e;
                }
